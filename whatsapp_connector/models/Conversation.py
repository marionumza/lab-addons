...
class AcruxChatConversation(models.Model):
    ...
    image_128 = fields.Binary("Avatar", attachment=True)
    ...
    @api.depends('image_128', 'res_partner_id.image')
    def _image_update(self):
        for rec in self:
            if rec.image_128 and rec.write_date:
                rec.image_url = get_image_url(self, rec, rec.image_128)
            elif rec.res_partner_id.image:
                rec.image_url = get_image_url(self, rec.res_partner_id, rec.res_partner_id.image)
            else:
