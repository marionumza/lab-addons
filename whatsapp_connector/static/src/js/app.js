odoo.define("acrux_chat.acrux_chat_translate", (function(require) {
    ...
    return ActionManager.include({
        _executeAction: function(action, options) {
            if (action.context.is_acrux_chat_room && "inline" == action.target) {
                ...
                this.actions[action.jsID] = action, action.action_ready = this._startController(controller).fail((() => {
    ...
    return Widget.extend({
        ...
        recordUpdated: function(env) {
            return this._fix_attach(), env && env.currentId && this.record[0] != env.currentId ? this.recordChange(env.currentId) : $.Deferred().resolve();
        },
    ...
    return Widget.extend({
        ...
        willStart: function() {
            return $.when(this._super(), this.getUserStatus());
        },
        start: function() {
    ...
    return Widget.extend({
        ...
        _initRender: function() {
            ...
            "audio" == this.ttype && (def = this.audioController()))), $.when(def);
        },
        destroy: function() {
        ...
        audioController: function() {
            ...
            return this.$audio.on("loadeddata", (_event => {
                ...
                this.$player.removeClass("o_hidden"), def.resolve();
    ...
    return Widget.extend({
        ...
        showMessages: function() {
            ...
            })) : $.Deferred().resolve(), this.$el.addClass("active"), "current" == this.status && this.messageSeen(), 
            def;
        },
        ...
        addExtraClientMessage: function(messages) {
            let show = this.parent.selected_conversation && this.parent.selected_conversation.id == this.id, def = $.Deferred().resolve(), $el = $("<div>");
            return messages && (messages = (messages = messages.map((r => new Message(this, r)))).filter((r => !this.messages_ids.has(r.id))), 
            ...
            })).always((() => framework.unblockUI()))), messages.length && def.then((() => {
        ...
        _syncLoop: function(i, arr, last_index, $el) {
            let out;
            return out = i < arr.length ? this._syncShow(i, arr, last_index, $el).then((() => this._syncLoop(i + 1, arr, last_index, $el))) : $.Deferred().resolve(), 
            out;
        },
        ...
        scrollConversation: function() {
            if (this.parent.$chat_message.children().size()) {
                let element = this.parent.$chat_message.children().last();
                element.children().size() && (element = element.children().last()), element[0].scrollIntoView();
            }
        },
        ...
        createMessage: function(options) {
            ...
            return options.custom_field && (json_data[options.custom_field] = !0), $.Deferred((def => {
                this._rpc({
                    model: "acrux.chat.conversation",
                    method: "send_message",
                    ...
                    msg.res_model = result.res_model, this.addCompanyMessage(msg), def.resolve(msg);
                }), (result => {
                    def.reject(result);
                }));
            }));
    ...
    return Widget.extend({
        ...
        sendMessage: function() {
            let out = $.Deferred().resolve(), options = {
                from_me: !0
        ...
        changeAttachment: function(event) {
            let self = this, files = event.target.files, attachments = self.get("attachment_ids");
            ...
            })), this.$("form.o_form_binary_form").submit(), this.$attachment_button.prop("disabled", !0);
        ...
        deleteAttachment: function(event) {
            event.stopPropagation();
            var $el, attachmentID, self = this;
            if (($el = $(event.target)).is("span") && ($el = $el.parent()), attachmentID = $el.data("id")) {
                var attachments = [];
                _.each(this.get("attachment_ids"), (function(attachment) {
                    attachmentID !== attachment.id ? attachments.push(attachment) : self._attachmentDataSet.unlink([ attachmentID ]);
    ...
    var core = require("web.core"), AbstractAction = require("web.AbstractAction"), session = require("web.session"), chat = require("acrux_chat.chat_classes"), config = require("web.config"), Dialog = require("web.Dialog"), field_utils = require("web.field_utils"), _t = core._t, chat_is_read = $.Deferred(), AcruxChatAction = AbstractAction.extend({
        template: "acrux_chat_action",
        events: {
        ...
        willStart: function() {
            return $.when(this._super(), this.getDefaultAnswers(), this.getRequiredViews(), this.getCurrency());
        },
        start: function() {
            return $.when(this._super(), session.is_bound).then((() => this._initRender().then(chat_is_read.resolve)));
        },
        _initRender: function() {
        ...
        selectConversation: function(event) {
            ...
            this.tabsClear()) : finish = $.Deferred().resolve(), this.toolbox.do_show(), this.toolbox.$input.focus(), 
        ...
        onInitConversation: function(conv_data) {
            if (conv_data.sellman_id[0] == session.uid) {
                ...
                conv ? def = $.Deferred().resolve() : (conv = new chat.Conversation(this, conv_data), 
