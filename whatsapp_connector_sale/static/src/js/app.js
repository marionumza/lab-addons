odoo.define("acrux_whatsapp_sale.res_partner", (function(require) {
    ...
    return FormView.extend({
        ...
        recordChange: function(res_partner_id) {
            return $.when(this._super(res_partner_id), this._rpc({
    ...
    return FormView.extend({
        ...
        _showAcruxFormView: function() {
            this._super().then((() => {
                if (this.moveSaleOrderNode(), !this.action.context.default_partner_id) {
                    let selector = "div.o_form_sheet > .o_notebook > ul.nav-tabs > li";
        ...
        recordChange: function(sale_order_id) {
            return $.when(this._super(sale_order_id), this._rpc({
                model: this.parent.model,
        ...
        moveSaleOrderNode: function() {
            ...
            main_group.size() && main_group.prev().hasClass("oe_title") && (main_group = main_group.detach(), 
    ...
    "use strict";
    var core = require("web.core"), Widget = require("web.Widget"), ajax = require("web.ajax"), QWeb = core.qweb;
    return Widget.extend({
        cssLibs: [ "/web/static/lib/nvd3/nv.d3.css" ],
        jsLibs: [ "/web/static/lib/nvd3/d3.v3.js", "/web/static/lib/nvd3/nv.d3.js", "/web/static/src/js/libs/nvd3.js" ],
        init: function(parent, options) {
        ...
        willStart: function() {
            return $.when(this._super(), ajax.loadLibs(this), this.getPartnerIndicator());
        },
        start: function() {
        ...
        graph_6last_sale: function() {
            let $out = $("<div>"), data = this.month_last_sale_data;
            return $out.addClass("o_graph_barchart"), this.chart = null, nv.addGraph((() => {
                let $svg = $out.append("<svg>").find("svg");
                $svg.height(150), this.chart = nv.models.discreteBarChart().x((d => d.label)).y((d => d.value)).showValues(!0).showYAxis(!1).showXAxis(!1).color([ "#1f77b4" ]).margin({
                    left: 40,
                    right: 40,
                    top: 10,
                    bottom: 30
                }), this.chart.xAxis.axisLabel(data[0].title), this.chart.yAxis.tickFormat((val => this.parent.format_monetary(val))), 
                this.chart.valueFormat((val => this.parent.format_monetary(val))), this.chart.tooltip.contentGenerator((key => QWeb.render("GraphCustomTooltip", {
                    color: key.color,
                    key: key.data.label,
                    value: this.parent.format_monetary(key.data.value)
                }))), d3.select($svg[0]).datum(data).transition().duration(600).call(this.chart), 
    ...
    AcruxChatAction.include({
        ...
        _initRender: function() {
            return $.when(this._super().then((() => {
                this.$tab_content_partner = this.$("div#tab_content_partner > div.o_group"), this.$tab_content_order = this.$("div#tab_content_order > div.o_group"), 
                this.$(".o_sidebar_right").find("ul.main-tabs.nav.nav-tabs").find("li > a").click((e => {
        ...
        tabPartner: function(event) {
            if (!$(event.currentTarget).hasClass("active") && this.selected_conversation && "current" == this.selected_conversation.status) {
        ...
        tabLastesSale: function(event) {
            if (!$(event.currentTarget).hasClass("active") && this.selected_conversation && this.selected_conversation.res_partner_id[0]) {
